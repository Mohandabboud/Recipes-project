...
function fastestFood () {
    ...
    fetch("https://api.spoonacular.com/recipes/random?apiKey="+fou  +"&number=20")
    .then(response => response.json())
    .then( data => {
        var number = 0;
        data.recipes.forEach( function (item) {
            fetch("https://api.spoonacular.com/recipes/"+ item.id +"/information?apiKey="+ fou  )
            .then(response => response.json())
            .then( recipe => { 
                var fastestFood = document.getElementsByClassName("fastestFood")[0];
...
function random () {
    fetch("https://api.spoonacular.com/recipes/random?apiKey="+ fou  +"&number=4")
    .then(response => response.json())
    .then( data => {
        var number = 0;
